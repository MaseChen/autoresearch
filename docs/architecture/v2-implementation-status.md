...
| R6 | Durable five-axis Campaign budgets, deadline/fencing checks, environment-bound staged lineage, trusted GPU-doctor quarantine clearance, OJ nomination/export/manual feedback, evidence-collected soak gates, three-database checkpoints, recovery, the Admin/Campaign maintenance fence, and bounded advisory profiling are implemented. ADR-009 adds a separate offline-built MetaX profiler image, split build/activation identities, durable worker outcome protocol, V2 toolchain/trace evidence, a no-child pre-soak canary, immutable per-recipe launch/host diagnostics, and terminalization paths that never replay an action. A5 persists bounded phase diagnostics; A6 freezes a 24 GiB hard limit plus host memory admission; A7 adds a bounded executable Triton cache; A8 normalizes the copied Python tree; A9 replaces no-IPC mode with one private, bounded shared-memory namespace. | A5's OOM action was safely abandoned. A6's noexec-cache known failure was terminalized. A7's native build failed safely on hostile checkout modes. A8 was built, activated and deployed; its compile and ten warmups passed, but tracked mcTracer lost completion while shared-memory RPC initialization failed under `--ipc=none`. Its UNKNOWN/RESERVED evidence was preserved and the quarantine was safely abandoned without replay. The A9 image was built, pulled by RepoDigest and qualified; the activation profile now pins it without changing build identity. Deployment verification and a new one-shot A9 canary remain outstanding. The 24/72/168-hour soak has not started and profiling collection remains gated. |
...
The A9 activation verification run completed 520 unit and
...
fixtures; they do not claim an A9 MetaX C500 canary, provider/OJ
