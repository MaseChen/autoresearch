...
| R6 | Durable five-axis Campaign budgets, deadline/fencing checks, environment-bound staged lineage, trusted GPU-doctor quarantine clearance, OJ nomination/export/manual feedback, evidence-collected soak gates, three-database checkpoints, recovery, the Admin/Campaign maintenance fence, and bounded advisory profiling are implemented. ADR-009 adds a separate offline-built MetaX profiler image, split build/activation identities, durable worker outcome protocol, V2 toolchain/trace evidence, and no-child pre-soak canary. | No 24/72/168-hour qualification soak has been run. Submit A's activation profile is deliberately inactive and uses a non-runnable image digest sentinel. A reviewed Linux/amd64 build must be pushed and pinned by Submit B without changing the build profile digest, then pass the trusted image canary before soak starts. |
