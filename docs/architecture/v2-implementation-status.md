...
| R6 | Durable five-axis Campaign budgets, deadline/fencing checks, environment-bound staged lineage, trusted GPU-doctor quarantine clearance, OJ nomination/export/manual feedback, evidence-collected soak gates, three-database checkpoints, recovery, the Admin/Campaign maintenance fence, and bounded advisory profiling are implemented. ADR-009 adds a separate offline-built MetaX profiler image, split build/activation identities, durable worker outcome protocol, V2 toolchain/trace evidence, a no-child pre-soak canary, immutable per-recipe launch/host diagnostics, and a doctor-gated operator-abandon path that never replays an UNKNOWN action. A5 persists bounded warmup/tracked subprocess diagnostics before sentinel parsing. A6 freezes a 24 GiB hard limit plus strict host `MemTotal`/`MemAvailable` admission checks without granting operational diagnostics scientific authority. | A5 was built, pushed, pulled, runtime-qualified, activated and run once. Its warmup was killed by the 4 GiB Docker memory cgroup; kernel evidence and the worker phase diagnostic agree, but the missing sentinel keeps the action UNKNOWN, RESERVED and quarantined. The A6 image has been built, pushed, pulled by exact RepoDigest and runtime/toolchain-qualified; its activation profile now pins the image while preserving the A6 build digest. It is not deployed and no A6 canary has run. The old A5 canary must be explicitly abandoned through the trusted recovery path before deployment. The 24/72/168-hour soak has not started and profiling collection remains gated. |
...
The A6 activation verification run completed 511 unit and integration tests
with 80.16% branch-aware coverage. `compileall` and the Git
whitespace check also passed. These checks use mocks and isolated SQLite/runtime
fixtures; they do not replace the separately sealed A6 image qualification or
claim a MetaX C500 canary, provider/OJ network access, or elapsed 24/72/168-hour
soak evidence.
