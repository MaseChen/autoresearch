...
| R6 | Durable five-axis Campaign budgets, deadline/fencing checks, environment-bound staged lineage, trusted GPU-doctor quarantine clearance, OJ nomination/export/manual feedback, evidence-collected soak gates, three-database checkpoints, recovery, the Admin/Campaign maintenance fence, and bounded advisory profiling are implemented. ADR-009 adds a separate offline-built MetaX profiler image, split build/activation identities, durable worker outcome protocol, V2 toolchain/trace evidence, a no-child pre-soak canary, immutable per-recipe launch/host diagnostics, and a doctor-gated operator-abandon path that never replays an UNKNOWN action. A5 additionally persists bounded warmup/tracked subprocess diagnostics before sentinel parsing without granting them completion authority. | A4 was pushed, requalified and activated, but its second trusted canary stopped during untraced warmup with no sentinel. A5 worker v3 has now been built, pushed, pulled by exact RepoDigest and runtime/toolchain-qualified; the separate activation profile pins that image while preserving the A5 build digest. It is not deployed and no A5 canary has run. The prior failed action remains RESERVED and quarantined until trusted doctor plus explicit abandonment. The 24/72/168-hour soak has not started and profiling collection remains gated. |
...
The A5 activation verification run completed 506 unit and integration tests
with 80.09% branch-aware coverage. `compileall` and
